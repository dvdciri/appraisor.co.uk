...
export default function StreetViewImage({ 
  ...
  const handleImageClick = () => {
    ...
    const fullAddress = `${address}, ${postcode}, UK`
    const encodedAddress = encodeURIComponent(fullAddress)
    const streetViewUrl = `https://www.google.com/maps/@?api=1&map_action=pano&viewpoint=${encodedAddress}`
  ...
  if (imageError) {
    return (
      <div 
        className={`${className} bg-gray-700/50 flex items-center justify-center cursor-pointer hover:bg-gray-600/50 transition-colors`}
  ...
  return (
    <div className="relative">
      ...
      <img
        ...
        className={`${className} ${isLoading ? 'hidden' : 'block'} cursor-pointer hover:opacity-80 transition-opacity`}
